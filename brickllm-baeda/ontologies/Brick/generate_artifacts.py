...
def generate_relationships():
    ...
    SELECT DISTINCT ?actualSource ?path ?actualTarget WHERE {
        ?sourceClass sh:property ?property .
        ?property sh:path ?basePath .
        ...
        {
            ?property sh:class ?targetClass .
        } UNION {
            ?property sh:or/rdf:rest*/rdf:first ?constraint .
            ?constraint sh:class ?targetClass .
        }

        ?actualSource rdfs:subClassOf* ?sourceClass .
        ?actualTarget rdfs:subClassOf* ?targetClass .

        FILTER(!isBlank(?actualSource))
        FILTER(!isBlank(?actualTarget))
        FILTER(!isBlank(?path))
    }
    """
    ...
    rules = {}
    for row in results:
        source = str(row.actualSource)
        path = str(row.path)
        target = str(row.actualTarget)

        if uris_to_exclude and (
                source.startswith(uris_to_exclude) or
                path.startswith(uris_to_exclude) or
                target.startswith(uris_to_exclude)
        ):
            continue

        if source not in rules:
            rules[source] = {}
        if path not in rules[source]:
            rules[source][path] = []
        if target not in rules[source][path]:
            rules[source][path].append(target)
